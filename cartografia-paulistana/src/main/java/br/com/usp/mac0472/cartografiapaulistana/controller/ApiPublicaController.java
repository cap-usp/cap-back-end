...
public class ApiPublicaController {
	...
	@GetMapping("/obras")
	public ResponseEntity<Page<ObraResponseDto>> getValidadas(Pageable pageable) {
		Page<Obra> obras = obraService.readObras(pageable, true, true);
		List<ObraResponseDto> obrasDto = obras.stream()
				.map(obra -> MapeadorUtil.mapObraToObraResponse(obra, mapper)).toList();
		Page<ObraResponseDto> response = PageableExecutionUtils.getPage(obrasDto, pageable,
